...
async function load() {
  const res = await apiCall("firewall.destination_nat_rules.list", {});
  if (res.Error === null) {
    rules = res.Data;
...
async function deleteRule() {
  const res = await apiCall("firewall.destination_nat_rules.delete", { index: selection[0] });
  if (res.Error === null) {
...
async function draggedRow(draggedRow: number, draggedOverRow: number) {
  ...
  const res = await apiCall("firewall.destination_nat_rules.move", {
    index: draggedRow,
  ...
  <div>
    ...
    >
      ...
      <router-link
        ...
        :to="'/firewall/destination_nat_rules/edit/' + selection[0]"
        >Edit</router-link
      >
