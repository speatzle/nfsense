...
async function load(){
  const res = await apiCall('firewall.inbound_rules.list', {});
  if (res.Error === null) {
    rules = res.Data;
...
async function deleteRule(){
  const res = await apiCall('firewall.inbound_rules.delete', { index: selection[0] });
  if (res.Error === null) {
...
async function draggedRow(draggedRow: number, draggedOverRow: number) {
  ...
  const res = await apiCall('firewall.inbound_rules.move', { index: draggedRow, to_index: draggedOverRow });
  if (res.Error === null) {
...
<template>
  <div>
    <TableView v-model:selection="selection" v-model:data="rules" title="Inbound Rules" :columns="columns" :loading="loading" :table-props="{draggable: true}" @dragged-row="draggedRow">
      <button @click="load">Refresh</button>
      <router-link class="button" to="/firewall/inbound_rules/edit">Create</router-link>
      <router-link class="button" :class="{ disabled: selection.length != 1 }" :to="'/firewall/inbound_rules/edit/' + selection[0]">Edit</router-link>
