...
/** Returns true if the value is null or undefined, matching the semantics of nullish coalescing */
export function isNullish(value: any) {
  return value === null || value === undefined;
}

/** Converts an enum value into just its variant string */
...
/** Primitively traverses an object to retrieve a nested value via a dot-separated path */
export function atPath(value: any, path: string): any {
  for (const segment of path.split(".")) value = (value ?? ({} as any))[segment];
  return value;
}
